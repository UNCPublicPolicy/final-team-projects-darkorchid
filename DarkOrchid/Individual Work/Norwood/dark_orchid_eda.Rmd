...
An overview of what these variables stand for:
 ...
 - mortality_ast: mortality from asthma per 100,000 
...
glimpse(data)
# character: location, county, state, name_of_nearest_plant 
# dbl: mortality_resp, perc_mortality_change_resp, mortality_ast, perc_mortality_change_ast, mortality_lung, perc_change_mortality_lung, lat, long, distance_to_nearest_plant
...
# perc_mortality_change_ast reads in as character but should be numeric, so here is the fix for that: 
data <- data %>% mutate(perc_mortality_change_ast = as.numeric(perc_mortality_change_ast))
glimpse(data)

# summary of NA values in the data set- most of them are in the AQI values 
